...
def render(config, dnsfile):
        ...
        for item in config['fabric-network'][tp]:
            ...
            if item.startswith('peer'):
                res.append({
                    ...
                    'yaml': tpl_peer.render(peer = config['fabric-network'][tp][item], name=item) 
                })
            ...
            if item.startswith('orderer'):
                res.append({
                'filename': "docker-compose-%s.yaml"%config['fabric-network'][tp][item]['host'],
                'yaml': tpl_orderer.render(orderer = config['fabric-network'][tp][item], name=item)
                })
                dnsfile.write(config['fabric-network'][tp][item]['host'] + '   ' + item + '\n')
    ...
    res.append({
        ...
        'yaml': tpl_configtx.render(orgs=configtx_conf['orgs'], orderers=configtx_conf['orderers'])
        })
